...
**Phase 4**：Hermes 优先整理（有 API Key 时）+ 每日免费配额桩；失败回退离线脚本。详见 `PHASE4.md`。

## 目录

- `miniprogram/` — 微信小程序（开发者工具导入）
- `server/` — FastAPI 整理服务（Hermes-first → `organize_vault.py`）
- `skills/obsidian-vault-organize/` — Hermes 技能
- `fixtures/messy-input/` — 样例乱笔记
- `PHASE1.md` … `PHASE4.md` — 阶段说明
...
安装与 DeepSeek 配置见 `PHASE2.md`（**不要把 API Key 提交进仓库**）。无本地 Hermes / Key 时 API 自动使用 `organize_vault.py`。

## 微信开发者工具

导入 `miniprogram/`，开发阶段关闭域名校验；API 默认 `http://127.0.0.1:8787`。小程序会持久化 `X-Client-Id` 并显示剩余配额。
